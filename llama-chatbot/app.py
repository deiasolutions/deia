    ...
    <head>
        ...
        <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            .panel-header h2 { font-size: 18px; margin-bottom: 10px; }
            .launch-btn {
                width: 100%;
                padding: 10px 16px;
                background: #667eea;
            ...
            .launch-btn:hover { background: #5568d3; transform: translateY(-1px); }
            .bot-list {
                flex: 1;
                overflow-y: auto;
                padding: 10px;
            ...
            .bot-item.active { background: #333; border-left-color: #28a745; }
            ...
            .bot-status-text { font-size: 12px; color: #aaa; margin-top: 4px; }
            .bot-actions {
                display: flex;
                gap: 4px;
                margin-top: 8px;
            ...
            .bot-action-btn:hover { background: #444; color: white; }
            ...
            .status-header h2 { font-size: 18px; }
            .status-list {
                flex: 1;
                overflow-y: auto;
                padding: 10px;
        ...
        <div class="main-container">
            ...
            <div class="bot-list-panel">
                <div class="panel-header">
                    <h2>🤖 Bots</h2>
                    <button id="launchBtn" class="launch-btn">+ Launch Bot</button>
                </div>
            ...
            <div class="chat-panel">
                ...
                <div class="chat-input-container">
                    <div class="chat-input-wrapper">
                        <input type="text" id="chatInput" class="chat-input" placeholder="Enter command or message..." autocomplete="off" disabled>
        ...
        <script>
            ...
            async function launchBot() {
                ...
                try {
                    ...
                    if (result.success) {
                        addMessage('assistant', `✓ Bot ${botId} launched successfully`);
                        refreshBotList();
                    } else {
                        addMessage('assistant', `✗ Failed to launch: ${result.error}`);
                    }
                } catch (error) {
            ...
            async function stopBot(botId) {
                ...
                try {
                    ...
                    const result = await response.json();
                    if (result.success) {
                        addMessage('assistant', `✓ Bot ${botId} stopped`);
                        refreshBotList();
                    ...
                    addMessage('assistant', `Error stopping bot: ${error.message}`);
                }
            }
            ...
            async function refreshBotList() {
                try {
                    ...
                    if (Object.keys(activeBots).length === 0) {
                        botList.innerHTML = '<div class="bot-item"><div class="bot-id">No bots running</div></div>';
                        return;
                    }
                    ...
                    for (const [botId, botData] of Object.entries(activeBots)) {
                        ...
                        botItem.innerHTML = `
                            ...
                            <div class="bot-actions">
                                <button class="bot-action-btn" onclick="selectBot('${botId}')">Select</button>
                                <button class="bot-action-btn" onclick="stopBot('${botId}')">Stop</button>
                            </div>
                        `;
            ...
            function addLoadMoreButton(total) {
                const btn = document.createElement('button');
                btn.textContent = `Load More History (${total} total messages)`;
                btn.style.cssText = 'background: #3a3a3a; color: #ccc; border: none; padding: 8px 16px; margin: 10px; cursor: pointer; border-radius: 4px;';
                btn.onclick = async function() {
                    const response = await fetch(`/api/chat/history?limit=200&bot_id=${selectedBotId}`);
                    const data = await response.json();
                    chatMessages.innerHTML = '';
                    data.messages.forEach(msg => displayHistoryMessage(msg));
                    this.remove();
                    btn.disabled = true;
                };
            ...
            function renderStatus() {
                ...
                for (const [botId, botData] of Object.entries(activeBots)) {
                    ...
                    statusItem.className = `status-item ${botData.status}`;
                    statusItem.innerHTML = `
                        <div class="status-label">${botId}</div>
                        <div class="status-value">Status: ${botData.status}</div>
                        <div class="status-value">Uptime: ${botData.uptime || 'N/A'}</div>
                        ${botData.current_task ? `<div class="status-value">Task: ${botData.current_task}</div>` : ''}
                    `;
                    statusList.appendChild(statusItem);
                }
            }

            // ========== MESSAGE ROUTING & CHAT ==========
            async function sendMessage() {
                ...
                try {
                    ...
                    const result = await response.json();
                    hideTypingIndicator();
                    addMessage('assistant', `${selectedBotId}: ${result.response || result.error}`);
                } catch (error) {
                    hideTypingIndicator();
                    addMessage('assistant', `Error: ${error.message}`);
                }
            ...
            // ========== EVENT LISTENERS ==========
            launchBtn.addEventListener('click', launchBot);
