...
def configure_callbacks(models_dir: Path, eval_log_dir: Path, eval_env: Monitor) -> CallbackList:
    """Configure checkpointing and evaluation callbacks tuned for the session."""
...
def make_directories() -> Tuple[Path, Path, Path]:
    ...
    for directory in (models_dir, logs_dir, tensorboard_dir):
        directory.mkdir(parents=True, exist_ok=True)

    return models_dir, logs_dir, tensorboard_dir


def train(total_timesteps: int, seed: int, resume: bool) -> Path:
    """Train the DQN agent and return the path to the saved best model."""
    models_dir, logs_dir, tensorboard_dir = make_directories()
    ...
    if resume and best_model_zip.exists():
        model = DQN.load(best_model_zip, env=train_env, device=device)
...
def evaluate(model_path: Path, episodes: int, eval_seed: int) -> List[int]:
    ...
    eval_env = build_env(PROJECT_ROOT / "results" / "logs" / "dqn_session" / "evaluation")
...
def evaluate_with_sb3(model_path: Path, episodes: int) -> Tuple[float, float]:
    """Evaluate the model with the SB3 helper to obtain reward statistics."""
    helper_env = build_env(PROJECT_ROOT / "results" / "logs" / "dqn_session" / "evaluation_helper")
    model = DQN.load(model_path)
    mean_reward, std_reward = evaluate_policy(model, helper_env, n_eval_episodes=episodes, deterministic=True)
    helper_env.close()
    return mean_reward, std_reward


def parse_args() -> argparse.Namespace:
    """Parse CLI arguments for the training session."""
    ...
    parser.add_argument(
        ...
        default=True,
        help="Resume training from an existing best_model.zip if present.",
    )
